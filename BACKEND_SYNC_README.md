...
| :-: | :--- | :--- | :--- | :-: |
| 1 | **Buwuhan** | Endpoint Standalone (`/buwuhans/standalone`) | Rute `GET` & `POST /buwuhans/standalone` untuk catatan mandiri (jadikan `invitation_id` nullable). | 🔴 Tinggi |
| 2 | **Ekspor** | Endpoint Streaming Export (`/export?format=xlsx\|csv`) | Endpoint streaming laporan Excel/CSV untuk Tamu, RSVP, Buwuhan, dan Admin Invitations. | 🟡 Sedang |
...
| **Catatan Buwuh** | CRUD Catatan Buwuh Undangan, Filter Nominal Uang, & Alamat Pemberi (`giverAddress`) | ✅ Sinkron |
...
| **Petugas Instant Link** | Magic Link Petugas (`/members/instant-link`, `/members/instant-access`), Scope `BUWUHAN_ONLY`, Hak Edit Mandiri, Larangan Hapus | ✅ Sinkron |
...
**Status:** ✅ **Sudah Selesai & Diterapkan di Backend dan Frontend**
- Tabel `buwuhan`: Kolom `giver_address` (text, nullable) telah aktif.
- Endpoint `GET /invitations/:id/buwuhans`, `GET /buwuhans/:id`, `GET /buwuhans`: Menyertakan `giverAddress`.
- Endpoint `POST /invitations/:id/buwuhans`, `PATCH /buwuhans/:id`: Menerima `giverAddress` pada request body.
...
5. **Konteks Acara Undangan:** Pada Panel Undangan, kolom isian acara undangan ditiadakan karena otomatis terikat langsung ke `invitationId` acara yang sedang dikelola.

## ✅ Checklist Prioritas Tim Backend

Silakan gunakan checklist berikut untuk pembaruan sprint backend:

- [x] **DB Migration:** Tambahkan kolom `giver_address` (text, nullable) di tabel `buwuhan` (Sudah Selesai)
- [ ] **DB Migration:** Pastikan kolom `invitation_id` di tabel `buwuhan` bersifat `NULLABLE`
- [x] **Schema Zod:** Tambahkan `giverAddress` (opsional) pada `createBuwuhanSchema` & `updateBuwuhanSchema` (Sudah Selesai)
- [x] **Serializer Buwuhan:** Sertakan `giverAddress` pada response `GET /invitations/:id/buwuhans` dan `GET /buwuhans/:id` (Sudah Selesai)
