...
const INVITATION_CATEGORIES: Array<{
...
}> = [
  {
    ...
    title: "Pernikahan",
    subtitle:
      "Akad nikah & resepsi perkawinan dengan data mempelai pria dan wanita.",
    badge: "Pernikahan",
...
const sectionClass =
  "rounded-2xl border border-slate-100 bg-slate-50/60 p-4 space-y-3";
...
function detectCategory(data?: ApiInvitation | null): InvitationCategory {
  ...
  if (titleLower.includes("aqiqah") || titleLower.includes("akikah"))
    return "aqiqah";
  return "wedding";
}
...
function toFormState(data: ApiInvitation): FormState {
  ...
  return {
    ...
    additionalNote:
      (data.additionalInfo as { note?: string } | null)?.note ?? "",
    dressCode:
      (data.additionalInfo as { dressCode?: string } | null)?.dressCode ?? "",
 ...
 */
export const InvitationForm = forwardRef<
...
>(function InvitationForm(
  ...
  const [selectedCategory, setSelectedCategory] =
    useState<InvitationCategory | null>(() => {
  ...
  function validate(): FormErrors {
    ...
    if (!form.title.trim()) next.title = "Judul undangan wajib diisi";
      ...
      next.title = "Judul maksimal 255 karakter";
    ...
    if (currentCategory === "wedding") {
      if (!form.groomName.trim())
        next.groomName = "Nama mempelai pria wajib diisi";
    ...
    if (form.venue.trim().length > 255)
      next.venue = "Nama lokasi maksimal 255 karakter";
    if (form.address.trim().length > 1000)
      next.address = "Alamat maksimal 1000 karakter";
    if (form.eventTime.trim().length > 100)
      next.eventTime = "Waktu maksimal 100 karakter";
  ...
  : null,
    ...
    if (form.additionalNote.trim() || form.dressCode.trim()) {
      payload.additionalInfo = {
        ...(form.additionalNote.trim()
          ? { note: form.additionalNote.trim() }
          : {}),
    ...
    try {
      ...
      if (isEditMode) {
        ...
        setPopupState({
          ...
          message:
            "Seluruh data dan perubahan informasi undangan telah berhasil disimpan.",
        });
      ...
      if (parsed.fieldErrors.eventTime)
        mapped.eventTime = parsed.fieldErrors.eventTime[0];
      if (parsed.fieldErrors.venue) mapped.venue = parsed.fieldErrors.venue[0];
      if (parsed.fieldErrors.address)
        mapped.address = parsed.fieldErrors.address[0];
      ...
      <div className="space-y-4 py-2">
        <div className="text-center space-y-1">
          ...
          <p className="text-xs text-muted max-w-md mx-auto">
            Silakan pilih jenis acara yang ingin Anda buat agar formulir dan
            rincian data disesuaikan dengan kebutuhan acara Anda.
          </p>
        </div>
  ...
  // =========================================================================
  const activeCatInfo =
    INVITATION_CATEGORIES.find((c) => c.id === currentCategory) ??
    INVITATION_CATEGORIES[0];
  ...
  return (
    <form id={id} onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <p className="text-[11px] text-muted">
            {currentCategory === "wedding" &&
              "Resepsi & Akad Nikah (Kategori: WEDDING)"}
            {currentCategory === "khitanan" &&
              "Tasyakuran Walimatul Khitan (Kategori: KHITANAN)"}
            {currentCategory === "rasulan" &&
              "Tasyakuran Rasulan / Merti Desa (Kategori: RASULAN)"}
            {currentCategory === "aqiqah" &&
              "Tasyakuran Kelahiran Ananda (Kategori: AQIQAH)"}
          </p>
        </div>
      ...
      <div>
        <label htmlFor="inv-dresscode" className={labelClass}>
          <span>Dress Code (opsional)</span>
        </label>
        <input
          id="inv-dresscode"
          type="text"
          ...
          placeholder="Batik / Nuansa Hijau Sage"
          className={fieldClass}
        />
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Mohon hadir tepat waktu. Parkir tersedia di sisi timur gedung."
            ...
            {templates.map((tpl) => (
              ...
              >
                ...
                <div className="w-full px-1 text-center">
                  <span
                    className="block truncate text-xs font-bold text-ink"
                    title={tpl.name}
                  >
                    {tpl.name}
                  </span>
