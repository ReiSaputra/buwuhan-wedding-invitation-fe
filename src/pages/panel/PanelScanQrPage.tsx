...
/** Data tamu hasil verifikasi QR yang menunggu konfirmasi petugas. */
type PendingGuest = {
  id: string;
  name: string;
...
export default function PanelScanQrPage() {
   ...
   */
  /**
   * Langkah 1: memverifikasi token QR ke backend dan menampilkan pratinjau
   * tamu. Kehadiran BELUM dicatat sampai petugas menekan "Konfirmasi Hadir".
   *
   * @param raw - Teks mentah dari kamera atau kotak input manual
   */
  const handleSubmitCode = useCallback(
    async (raw: string) => {
      ...
      try {
        const guest = await verifyQr(qrCode);
        setPendingQr(qrCode);
        setPendingGuest({
          id: guest.id,
          name: guest.name,
        ...
        setFeedback({
          tone: "error",
          text: "Kode QR tidak dikenali pada undangan ini.",
        });
  ...
  /** Langkah 2: mencatat kehadiran setelah petugas mengonfirmasi pratinjau. */
  const handleConfirmCheckIn = useCallback(async () => {
    if (!pendingQr || isMutating) return;
    try {
      const guest = await checkInByQr(pendingQr);
      setFeedback({
        tone: "success",
      ...
      setFeedback({
        tone: "error",
        text: "Gagal mencatat kehadiran. Tamu mungkin sudah check-in.",
      });
    } finally {
      setPendingGuest(null);
      setPendingQr("");
  ...
  }, [pendingQr, checkInByQr, isMutating]);
    ...
    <div className="animate-in fade-in space-y-6 duration-300">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <StatCard
          ...
          value={formatNumber(stats.totalAttended)}
          icon={<CheckCircle2 size={18} />}
          variant="filled"
          hint={`${formatNumber(stats.totalPaxActual)} orang hadir`}
        />
        <StatCard
          label="Belum Hadir"
        ...
        <div className="grid items-start gap-6 md:grid-cols-2">
          ...
          <div className="space-y-4 rounded-3xl border border-border bg-slate-900 p-6 text-center text-white shadow-xl">
            {isCameraOn ? (
              <QrScanner
                isActive={isCameraOn}
                onDecoded={(value) => void handleSubmitCode(value)}
              />
            ) : (
          ...
          <div className="space-y-4">
            ...
            {pendingGuest && (
              <div className="animate-in zoom-in-95 space-y-3 rounded-3xl border-2 border-primary/30 bg-white p-6 shadow-lg">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <Badge variant="default">
                    {pendingGuest.category ?? "Tanpa Kategori"}
                  </Badge>
                  <Badge variant="default">{pendingGuest.paxCount} pax</Badge>
                  {pendingGuest.isAttended && (
                    <Badge variant="warning">Sudah pernah check-in</Badge>
                  )}
                </div>
            ...
            >
              ...
              <p className="text-xs text-muted">
                Bila kamera bermasalah, tempel token QR atau tautan undangan
                tamu di sini.
              </p>
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  variant="primary"
                  size="sm"
                  disabled={!manualCode || isMutating}
                >
                  {isMutating ? "Memproses…" : "Cek Kode"}
                </Button>
              </div>
            </form>
          </div>
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {recentCheckIns.map((guest) => (
                <tr key={guest.id} className="transition hover:bg-slate-50/70">
                  ...
                  <td className={tdClass}>
                    {guest.paxActual ?? guest.paxCount} orang
                  </td>
                  <td className={tdClass}>
                    {guest.checkedInAt ? formatTimeWib(guest.checkedInAt) : "-"}
                  </td>
                  <td className={tdClass}>
                    <Button
                      variant="outline"
                      ...
                      icon={<LogOut size={13} />}
                      disabled={isMutating}
                      onClick={() => void checkOutByGuestId(guest.id)}
                    >
                      Check-out
                    </Button>
                  </td>
